...
class Regression(nn.Module):
    def __init__(self):
        ...
        self.fuse1 = nn.Sequential(
            Conv2d(1024+512+1, 256, 1),
            Conv2d(256, 256, 3)
        ) # 16*

        self.fuse2 = nn.Sequential(
            Conv2d(256+256+1, 128, 1),
            Conv2d(128, 128, 3)
        ) #8*
        ...
        self.dsn1 = Conv2d(1024+512, 1, 1, bn=True)
        self.dsn2 = Conv2d(256+256, 1, 1, bn=True)
        self.msn = Conv2d(128, 1, 1, bn=True, se='sigmoid')
    ...
    def forward(self, x1, x2, x3):
        ...
        s1 = torch.cat((x3, x2), dim=1)
        del x2, x3
        d1 = self.dsn1(s1)
        s1 = self.fuse1(torch.cat((s1, d1), 1))
        s1 = F.interpolate(s1, scale_factor=2, mode='bilinear')
        s1 = torch.cat((s1, x1), dim=1)
        del x1
        d2 = self.dsn2(s1)
        s1 = self.fuse2(torch.cat((s1, d2), 1))
        # d3 = self.dsn3(s2)
        mask = self.msn(s1)
        s1 = self.fuse3(s1 * mask)

        return s1, [d1, d2], mask
...
class AdapConv_group(nn.Module):
    def __init__(self, num_heads, ws=16):
        super(AdapConv_group, self).__init__()
        ...
        self.get_mask = nn.Sequential(
            nn.Conv1d(num_heads, 1, 1),
            nn.ReLU(inplace=True)
        )
        self.get_mask1 = nn.Sequential(
            nn.Linear(ws*ws, 1),
            nn.Sigmoid()
        )
    ...
    def forward(self, attn, h_group, w_group):
        '''
        input:
             weight: B, hgwg, n_head, ws*ws, ws*ws
        '''
        B, group_num,num_heads, N1,N2 = attn.shape
        # attn=attn.view(-1,1,N1,N2)
        # attn = self.Adp(attn).squeeze(1) # Bhw, n_head, ws*ws
        # attn = attn.view(B,group_num,num_heads, N1,N2).contiguous()
        attn1 = attn.transpose(2, 3).contiguous().reshape(B, h_group, w_group, self.ws, self.ws, self.num_heads,
                                                         self.ws * self.ws)
        attn1 = attn1.transpose(2, 3).reshape(-1, self.num_heads, self.ws * self.ws).contiguous()
        attn1 = self.Adp(attn1)#BHW,head,ws*ws
        mask = self.get_mask(attn1)  # Bhw,1,ws*ws
        mask = self.get_mask1(mask.squeeze(1).reshape(B, -1 , self.ws * self.ws))
        # one = torch.ones_like(attn, device=attn.get_device()) * (-1000000)

        attn1= attn1.view(B,h_group,self.ws,w_group,self.ws, self.num_heads, N2).contiguous().transpose(2,3).reshape(B,group_num, N1,num_heads,N2).transpose(2,3).contiguous()
        # one = torch.ones_like(attn) * (-1000000)
        one = torch.ones_like(attn1, device=attn1.get_device()) * (-1000000)
        attn = torch.where(attn1 > 0, attn, one)
        return mask, attn
...
class GroupAttention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., ws=1):
        ...
        self.ws = ws
        self.get_mask = AdapConv_group(self.num_heads, self.ws)
    ...
    def forward(self, x, H, W):
        ...
        x = x.reshape(B, h_group, self.ws, w_group, self.ws, C).transpose(2, 3)  # b,h_group,w_group,ws,ws,C 1,16,16,8,8,128
        qkv = self.qkv(x).reshape(B, total_groups, -1, 3, self.num_heads, C // self.num_heads).permute(3,0,1,4,2,5)
        # B, hgwg, ws*ws, 3, n_head, head_dim -> 3, B, hgwg, n_head, ws*ws, head_dim
        q, k, v = qkv[0], qkv[1], qkv[2]  # B, hgwg, n_head, ws*ws, head_dim 1,256,4,64,32
        del qkv,x
        attn = (q @ k.transpose(-2, -1)) * self.scale  # B, hgwg, n_head, ws*ws, ws*ws 1,256,4,64,64
        # print ("*****///",attn.shape)
        mask, attn =self.get_mask(attn, h_group, w_group)
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)  # attn @ v-> B, hw, n_head, ws*ws, head_dim -> (t(2,3)) B, hw, ws*ws, n_head,  head_dim
        attn = (attn @ v).transpose(2, 3).contiguous().reshape(B, h_group, w_group, self.ws, self.ws, C)
        attn = attn.transpose(2, 3).reshape(B, N, C).contiguous()
        attn = self.proj(attn)
        attn = self.proj_drop(attn)
        # print ("**********8grou[p",mask.shape,attn.shape)
        return mask * attn
...
class CPVTV2(PyramidVisionTransformer):
    ...
    def forward(self, x):
        ...
        x = self.forward_features(x)
        p, d1,d2 = self.regression(x[0],x[1],x[2])
        return p, d1,d2
...
@register_model
def AMM4(pretrained=True, **kwargs):
    model = ALTGVT(
